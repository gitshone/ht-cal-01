...
import { CreateEventDto } from '@ht-cal-01/shared-types';
import moment from 'moment';

interface CreateEventModalProps {
  isOpen: boolean;
  onClose: () => void;
  onCreateEvent: (eventData: CreateEventDto) => Promise<void>;
}
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorData = error as {
        response?: {
          data?: { fieldErrors?: Record<string, string>; error?: string };
        };
      };
      if (errorData?.response?.data?.fieldErrors) {
