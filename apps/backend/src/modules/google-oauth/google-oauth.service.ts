...
import { NoGoogleTokensError } from '../../core/errors/http.errors';

export class GoogleOAuthService extends BaseService {
  private oauth2Client: any;

  constructor(private googleOAuthRepository: GoogleOAuthRepository) {
    super();
  ...
  async exchangeCodeForTokens(userId: string, code: string): Promise<void> {
    try {
      ...
      // Save tokens to user
      await this.googleOAuthRepository.updateTokens(userId, {
  ...
  async getOAuth2Client(userId: string): Promise<any> {
    try {
      const user = await this.googleOAuthRepository.getUserTokens(userId);
  ...
  async revokeTokens(userId: string, token: string): Promise<void> {
    try {
      ...
      await this.googleOAuthRepository.revokeTokens(userId);
