...
export class CacheService extends BaseService {
  ...
  private hashObject(obj: Record<string, any>): string {
    const hashString = Object.keys(obj)
      .sort()
  ...
  async invalidateEventCache(userId: string, eventId?: string): Promise<void> {
    try {
      // Invalidate all event-related caches for the user
      const patterns = [
  ...
  async clearAllCache(): Promise<number> {
    try {
      const keys = await redis.keys(`${this.DEFAULT_PREFIX}*`);
      if (keys.length === 0) {
        return 0;
      }

      const deletedCount = await redis.del(...keys);
      this.logInfo('All cache cleared', { deletedCount });
      return deletedCount;
    } catch (error) {
      this.logError('Clear all cache error', error as Error);
      return 0;
    }
  }
